                    ...
                    for(;;) {
                        ...
                        if ($delimiter === '`' && $this->a === "\n") {
                            ...
                            throw new W3JSMin_UnterminatedStringException(
                                esc_html("JSMin: Unterminated String at byte {$byte}: {$str}"));
                        }
                        $str .= $this->a;
                        if ($this->a === '\\') {
                            $this->output .= $this->a;
                ...
                if ($this->b === '/' && $this->isRegexpLiteral()) {
                    ...
                    for(;;) {
                            ...
                            for(;;) {
                                ...
                                if ($this->isEOF($this->a)) {
                                    throw new W3JSMin_UnterminatedRegExpException(
                                        esc_html("JSMin: Unterminated set in RegExp at byte "
                        ...
                        if ($this->a === '/') { // end pattern
                            ...
                            $byte = $this->inputIndex - 1;
                            throw new W3JSMin_UnterminatedRegExpException(
                                esc_html("JSMin: Unterminated RegExp at byte {$byte}: {$pattern}"));
                        }
                        $this->output .= $this->a;
                        $this->lastByteOut = $this->a;
                    }
        ...
        for(;;) {
            ...
            if ($get === '*') {
                ...
                throw new W3JSMin_UnterminatedCommentException(
                    esc_html("JSMin: Unterminated comment at byte {$this->inputIndex}: /*{$comment}"));
            }
            $comment .= $get;
        }
    }
